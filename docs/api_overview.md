...
- low-level pull API と high-level subscription API の両方を提供する
...
SimNode を作る
  ...
  -> internal SHM endpoint で subscription を使っていれば tick() が dispatch_recv_events() も行う
  -> 終了時に shutdown する

WebSocket 等を使う場合:
  -> 独立した EndpointNode を追加する
  -> SimNode とは別に open / start / dispatch_recv_events または recv / send する
```
...
### 1. 受信は caller-controlled を基本にする

Godot 側の受信は、利用者または framework が Godot main loop から明示的に進める形にする。
...
この方針の上で、`HakoniwaEndpointNode` は 2 つの層を提供する。
...
high-level API も内部では caller-controlled であり、native callback から直接 scene tree は触らない。

### 2. `process_recv_events()` / `dispatch_recv_events()` を main loop から呼べるようにする
...
- low-level pull API を使う場合:
  - `process_recv_events()`
  - `recv_next_*()`
- high-level subscription API を使う場合:
  - `dispatch_recv_events()`
...
そのため、high-level subscription API でも native callback は内部 queue に積むだけに留め、実際の callback / signal 発火は `dispatch_recv_events()` で行う。
...
- endpoint を `open(...)` する
- low-level pull API を使うなら必要に応じて `set_recv_event(...)` を設定する
- high-level subscription API を使うなら `create_subscription_*()` を設定する
- `start()` する
- transport に応じて `post_start()` を呼ぶ余地を残す
- high-level subscription API を使う場合、対象 entry の `notify_on_recv: true` を endpoint JSON に設定する

### `_process()` または `_physics_process()`

- low-level pull API の場合:
  - `process_recv_events()` を呼ぶ
  - `recv_by_name(...)` または `recv_next()` を呼ぶ
- high-level subscription API の場合:
  - `dispatch_recv_events()` を呼ぶ
- 受信した結果を上位ノードへ渡す
