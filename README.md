...
Godot 側は完成済み project ではなく、既存 project に `sample.gd` と `config/` を持ち込んで試す。
この example は `std_msgs/UInt64` を双方向に 1 本ずつ流す最小構成です。詳細は [examples/python_pdu_minimal/README.md](examples/python_pdu_minimal/README.md) を参照してください。
...
`tools/run_python_pdu_minimal_controller.sh` は Python 側に `config/endpoint_shm_with_pdu_python.json` を渡す。`endpoint_shm_with_pdu.json` は Godot endpoint 用設定であり、Python 側には使わない。
